...
BEGIN
  ...
  PROCESS(ss_n, clk, st_load_en, tx_load_en, rx_req, trdy)
  BEGIN
    ...
    ELSE
      FOR i IN 0 TO d_width-1 LOOP          
        IF(falling_edge(clk)) THEN
				IF(wr_add = '0' AND bit_cnt(i+9) = '1') THEN
    ...
    ELSIF(rising_edge(clk)) THEN
		IF(rd_add = '0' AND bit_cnt(7 DOWNTO 0) = "00000000" AND bit_cnt(d_width+8) = '0') THEN
    ...
    IF(ss_n = '1' OR reset_n = '0') THEN           --no transaction occuring or reset
      miso <= 'Z';
    ELSIF(rising_edge(clk)) THEN  --write status register to master
      IF(rd_add = '1') THEN
